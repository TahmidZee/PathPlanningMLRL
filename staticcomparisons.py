# -*- coding: utf-8 -*-
"""StaticComparisons.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ySIwNT6ipih4yShkDS81rODHrr_tY_CT
"""
...
def take_action(grid, state_index, action, goal):
        ...
        state = (state_index // grid_shape[1], state_index % grid_shape[1])  # Convert index to grid position

        # Define the action effects (up, down, left, right)
        action_effects = [(-1, 0), (1, 0), (0, -1), (0, 1)]

        # Calculate new state position
        new_state = (state[0] + action_effects[action][0], state[1] + action_effects[action][1])

        # Check bounds and obstacles
        if (0 <= new_state[0] < grid_shape[0] and 0 <= new_state[1] < grid_shape[1] and grid[new_state] == 0):
            new_state_index = new_state[0] * grid_shape[1] + new_state[1]
            done = new_state == goal
            reward = 100 if done else -1  # Reward for reaching the goal, else a step penalty
        else:
            new_state_index = state_index  # Remain in the same state if out of bounds or hitting an obstacle
            reward = -10  # Penalty for hitting an obstacle or going out of bounds
            done = False

        return new_state_index, reward, done

def run_q_learning(grid, start, goal, max_steps_per_episode=100):
